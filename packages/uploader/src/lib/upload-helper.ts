...
import { AjaxResponse, AjaxError } from './ajax-request';
import { FileRecord, ConfigureFn, CreateFormDataFn } from '@file-agent/core';
...
class UploadHelper {
  ...
  public prepareUploadError(fileRecord: FileRecord, err: AjaxError, timeout?: number) {
    ...
    // fileRecord.upload.data = undefined; // will cause trouble when "trying again"
  ...
  public upload(
    ...
    return new Promise((resolve, reject) => {
      Promise.all(promises).then((responses) => {
        if (failedUploadsCount && failedUploadsCount === promises.length) {
          // all uploads failed
          reject(responses);
          return;
        }
